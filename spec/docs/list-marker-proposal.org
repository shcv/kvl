...
1. *Serialization Preference* - Resolved: =dumps()= accepts a =list_marker= parameter to control output format. =None= auto-detects from config, empty string forces categorical.
2. *Mixed Usage* - Resolved: multiple markers (e.g., =-+*=) can be declared; each list can use any declared marker. Mixing markers within a single list is valid.
3. *Schema Integration* - Open: how schemas indicate preference for list markers is not yet defined.
4. *Error Messages* - Resolved: parse errors include line numbers and describe the issue (e.g., "no space after marker").

* Migration Path (completed)

All phases completed. List markers fully implemented across Python, Go, Zig, and JavaScript.
