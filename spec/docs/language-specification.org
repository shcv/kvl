...
The parser captures an entire indented block as a raw string. Only indentation level matters — if a continuation line is indented deeper than the parent key, it is part of the value. The parser does NOT examine separator characters during this stage.

*** Stage 2: Model Re-parse (Fix-point)

The captured block is re-parsed as key-value pairs at the block's indentation level:
- If *all* lines parse as =key {sep} value= → the block becomes a *nested structure*
- If *any* line fails to parse → the entire block is a *plain text string*

This is all-or-nothing: mixed blocks (some lines with separators, some without) are always plain text.

*** Examples

#+begin_src kvl
/= Nested structure (all continuation lines have separator)
server =
    host = localhost
    port = 8080
...
- =loads()= (high-level): strips leading =\n= and dedents to the block's indentation level
