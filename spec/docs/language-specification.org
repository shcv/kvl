...
a\\=b              # \ before = → escape → no separator → bare key "a\=b"
#+end_src
...
*Implementation Note:* Implementations should use a two-pass approach: first, find the unescaped separator (using backslash counting) to split key/value; second, unescape by removing backslashes that immediately precede the separator character.
