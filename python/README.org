...
- *Pythonic*: Schema-driven validation and list/dict transformations
...
# Save and load files
kvl.dump(config, "config.kvl", public_format=True)
config = kvl.load("config.kvl")
#+END_SRC

The public serializer keeps ordinary scalars readable, writes =None= as the
literal text =null=, and uses repeated keys for flat scalar lists:
...
config = kvl.load("config.kvl")
kvl.dump(config, "output.kvl", public_format=True)
#+END_SRC
...
kvl convert input.kvl out.json      # KVL to JSON (file)
...
# Validate and type-convert through a schema
